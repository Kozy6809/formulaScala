...
public class MainView extends JFrame {
   ...
   */
	private void init() {
		...
		m.add(mi);
		/*
		 * m = new JMenu("一括処理(G)"); m.setMnemonic('G'); mb.add(m); mi = new
		 * JMenuItem("資材の一括変更(M)"); mi.setMnemonic('M'); mi.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent e) {
		 * mvc.showMGU(); } }); m.add(mi);
		 */
		m = new JMenu("ウィンドウ(W)");
		m.setMnemonic('W');
		mb.add(m);
		...
		Border paneBorder = BorderFactory.createCompoundBorder(emp,
		...
		codeField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				mc.searchByCode(seriesList.getSelectedValues(),getSearchCode(), searchMode);
			}
		});
		KeyListener resetColor = new KeyListener() {
			@Override
		...
		nameField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				getResultList().clearSelection();
				// mvc.clearResult();
				mc.searchByName(seriesList.getSelectedValues(),getSearchName(), searchMode);
			}
		});
		p2.add(nameField);
		codeField.addKeyListener(resetColor);
		nameField.addKeyListener(resetColor);
		p1 = new JPanel(new BorderLayout());
		p0.add(p1, BorderLayout.CENTER);
		...
		b.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				mc.showFBrowser(resultList.getSelectedIndices());
			}
		});
		p2.add(b);
	...
	public void setResultModel(AbstractListModel<String> m) {
		getResultList().setModel(m);
	}

	public void setSeriesModel(AbstractListModel<String> m) {
		getSeriesList().setModel(m);
	}
}
/*
