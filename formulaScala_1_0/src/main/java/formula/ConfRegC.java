...
public class ConfRegC implements IConsts, IQueryClient {
    ...
    private ConfRegM cr;
    private ConfRegV crv;
    private Vector<Boolean> checks; // 登録のチェックマークの値の並び
    ...
    private class CRTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int column) {
            switch (column) {
                ...
                case 2:
                    return cr.getData(row, 1) + " " + cr.getData(row, 2) + " " + cr.getData(row, 3);
                case 3:
        ...
        void setValueAt(Boolean aValue, int rowIndex, int columnIndex) {
    ...
    public void queryCallBack(Object o, int mode) {
        ...
        switch (mode) {
            case 1:
                ...
                break;
            case 2:
                Runnable r = () -> cr.load(ConfRegC.this, 1);
     ...
     */
    public void update() {
        Vector<Integer> pcodes = new Vector<>();
        for (int i=0, n=checks.size(); i < n; i++) {
            if (checks.elementAt(i)) {
                pcodes.addElement((Integer)cr.getData(i, 1));
            }
        }
