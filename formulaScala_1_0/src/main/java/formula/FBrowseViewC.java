...
public class FBrowseViewC implements IConsts, IWinControl {
    ...
    /**
     * 印刷を実行する Java2D version
     *
     * @param showNorm boolean
     */
    public void print2(boolean showNorm) {
        if (editing) syncModel(new Date());
        FormulaPrintable fp = new FormulaPrintable();
        if (showNorm) {
            fp.setModeResolv(false);
            fp.setMr(fm.getNormForPrint());
        } else {
            fp.setModeResolv(true);
            fp.setMr(fm.getDecompForPrint());
        }
        fp.setPname(title);
        fp.setComment(fm.getComment());
        fp.setPerson(fm.getPerson());
        fp.setReason(fm.getReason());
        fp.setRegDate(DateFormat.getDateInstance().format(fm.getDate()));
        NumberFormat nf = NumberFormat.getInstance();
        nf.setMinimumFractionDigits(3);
        nf.setMaximumFractionDigits(3);
        fp.setSg(nf.format(fm.getSG()));
        if (!fm.isPriceValid()) {
            fp.setPrice("原料単価不正");
        } else {
            double price = fm.getPrice();
            if (price < 0.0) fp.setPrice("---");
            else {
                nf.setMinimumFractionDigits(3);
                nf.setMaximumFractionDigits(3);
                fp.setPrice(nf.format(price));
            }
        }

        Print p = new Print(fp);
        try {
            p.print();
        } catch (Exception e) {
